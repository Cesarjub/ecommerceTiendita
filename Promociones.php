<html>
  <head>
    ...
    <!-- Titulo de la pagina -->
    <title>Tiendita | Promociones</title>
...
<body>
        ...
        <!-- Baner de pagina -->
        <section id = "seccion-prom" class = "banner-sc">
...
$dbh = $nuevaConexion -> conectar();
        ...
        //Consulta
        $sqlCategorias =  $dbh ->query("
        SELECT m.NOMBRE_MARCAS, COUNT(p.ID_MARCAS) AS TOTAL_MARCA 
        FROM marcas m 
        LEFT JOIN productos p 
        ON m.ID_MARCAS = p.ID_MARCAS
        WHERE p.ID_CT = 2
        GROUP BY m.ID_MARCAS
        ORDER BY COUNT(p.ID_MARCAS) DESC
        ;");
        ...
        ?>
                  ...
                  <li class = "">
                    <a category = "all" class = "text-capitalize text-decoration-none categoryItem">Todos</a>
                  </li>
                  <?php  

                  while($datosCategoria = $sqlCategorias->fetch(PDO::FETCH_ASSOC)) 
                  {
                  ?> 
										<li class = "">
                      <a category = "<?php echo  strtolower($datosCategoria['NOMBRE_MARCAS']) ?>"  class = "text-capitalize text-decoration-none categoryItem">
                        <?php echo strtolower($datosCategoria['NOMBRE_MARCAS'])." "."(".strtolower($datosCategoria['TOTAL_MARCA']).")"?>
                      </a>
                    </li>
                  <?php  
                  }
								...
								<div class = "shop-top">
									<div class = "shop-shorter">
										<div class = "single-shorter">
											<label>Artículos por página:<span class = "fw-normal"> 24</span></label>
										</div>

                    </div>
									<ul class = "view-mode">
										<li class = "active"><a href = "#">
                    <svg xmlns = "http://www.w3.org/2000/svg" width = "22" height = "22" fill = "currentColor" class = "bi bi-grid-fill mt-1" viewBox = "0 0 16 16">
    ...
    <?php         
        ...
        //Consulta
        $sqlProductos =  $dbh ->query("
        SELECT p.*, m.NOMBRE_MARCAS FROM productos p INNER JOIN marcas m ON m.ID_MARCAS = p.ID_MARCAS WHERE p.ID_CT = 2 ORDER BY p.ID_P
        ;");

        //Listar datos
        while($datos = $sqlProductos->fetch(PDO::FETCH_ASSOC)) 
        {

        ?>

            <!-- Productos -->
            <div class = "col-lg-4 col-md-6 col-12 mt-2 product-item" category = "<?php echo  strtolower($datos['NOMBRE_MARCAS']) ?>">
                <div class = "product-grid3">
                <div class = "product-image3">
                    <a href = "./ProductosDetalle.php?ref=<?php echo  $datos['ID_P'] ?>" class = "ver-producto">
                        <img class = "pic-1" src = "data:image/jpg;base64,<?php echo  base64_encode($datos['IMAGEN']) ?>" alt = "Producto">
                        <!--    <img class = "pic-2" src = "data:image/jpg;base64, '.base64_encode($datos['IMAGEN']).'" alt = "producto">-->
                    </a>

                    <ul class = "social">
                        <li>
                            <a href = "./ProductosDetalle.php?ref=<?php echo  $datos['ID_P'] ?>" >
                    ...
                    <div class = "product-content">
                        <h3 class = "title"><a href = "./ProductosDetalle.php?ref=<?php echo  $datos['ID_P'] ?>" class = "text-capitalize text-decoration-none fs-6"><?php echo $datos['NOMBRE_P'] ?></a></h3>
                        <div class = "price">
                        <?php echo '$' .$datos['PRECIO_PUBLICO'] ?>
                        <!--<span>$75.00</span>-->
                        </div>
                    </div>
    ...
    <!-- JavaScript - Filtros productos -->
    <script src = "JS/Interfaz/FiltrosCategoria.js" type = "text/javascript"></script>
